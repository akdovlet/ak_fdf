...
/*   Updated: 2025/02/23 01:36:13 by akdovlet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotation_matrix(t_grid *grid, t_pixel **pixel)
{
	...
	while (y < grid->lines)
	{
		...
		while (x < grid->rows)
		{
			rotate_z(&pixel[y][x], angle_z * M_PI / 180.0);
			rotate_x(&pixel[y][x], angle_x * M_PI / 180.0, grid->z);
			rotate_y(&pixel[y][x], angle_y * M_PI / 180.0);
